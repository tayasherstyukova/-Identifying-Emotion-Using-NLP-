...
print("Model Keeping Stopwords")
mnb = MultinomialNB()
mnb.fit(X_train_vectors_tfidf, y_train)
predicted = mnb.predict(X_test_vectors_tfidf)
...
print("Model Removing Stopwords")
'''Call finalpreprocess() if you want to keep stopwords
Call finalpreprocess_stop() to remove stopwords'''
...
X_train_ns, y_train_ns = train.clean_text, train.emotion
X_test_ns, y_test_ns = test.clean_text, test.emotion
X_val_ns, y_val_ns = val.clean_text, val.emotion
...
'''Call for tfidf vs countvect'''
#TFIDF
X_train_vectors_tfidf_ns = tfidf_vectorizer.fit_transform(X_train_ns) #tfidf runs on non-tokenized sentences 
# Only transform x_test (not fit and transform)
X_test_vectors_tfidf_ns = tfidf_vectorizer.transform(X_test_ns) #Don't fit() your TfidfVectorizer to your test data: it will 
#change the word-indexes & weights to match test data. Rather, fit on the training data, then use the same train-data-
#fit model on the test data, to reflect the fact you're analyzing the test data only based on what was learned without 
#it, and the have compatible
X_val_vectors_tfidf_ns = tfidf_vectorizer.transform(X_val_ns)

mnb = MultinomialNB()
mnb.fit(X_train_vectors_tfidf_ns, y_train_ns)
predicted_ns = mnb.predict(X_test_vectors_tfidf_ns)
acc_score_ns = metrics.accuracy_score(predicted,y_test_ns)
prec_score_ns = precision_score(y_test_ns,predicted_ns, average='macro')
recall_ns = recall_score(y_test_ns, predicted_ns,average='macro')
f1_ns = f1_score(y_test_ns, predicted_ns,average='macro')
matrix_ns = confusion_matrix(y_test_ns,predicted_ns)

print(str('Accuracy: '+'{:04.2f}'.format(acc_score_ns*100))+'%')
print(str('Precision: '+'{:04.2f}'.format(prec_score_ns*100))+'%')
print(str('Recall: '+'{:04.2f}'.format(recall_ns*100))+'%')
print('F1 Score: ',f1_ns)
print('\n')
print(classification_report(y_test_ns,predicted_ns))